...
#<output>:     normalize gray scale image
def normalize(img):
...
def question1():
    img  = cv2.cvtColor(cv2.imread('./orig_images/Jennifer.jpg'), cv2.COLOR_BGR2GRAY)
    cv2.imwrite("./question1/orig_img.jpg", img);

    #apply box kernel
    box_size     = len(img) // 100
...
def question2():
    ...
    sobel_img = cv2.filter2D(img, -1, sobel_px) + cv2.filter2D(img, -1, sobel_py)
    ...
    robert_img = cv2.filter2D(img, -1, robert_px) + cv2.filter2D(img, -1, robert_py)
...
def question3():
    ...
    second_deri  = np.float32([[1,1,1],[1,-8,1],[1,1,1]])
...
def question4():
    ...
    gaussian_filter   = produce_gaussian_kernel(5, 0.0070)
    gaussian_img     = cv2.filter2D(img, -1, gaussian_filter)
    subtracted_img   = img - gaussian_img
...
#q1: apply convolution or correlation?
#q2: rober cross?
#q4:how to tune parameter?
#g: what is high pass / low pass
#g:lec 19-p18
