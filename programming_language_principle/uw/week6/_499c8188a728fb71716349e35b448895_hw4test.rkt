...
(define a 2)
(define cache (cached-assoc (list (cons 1 2) (cons 3 4)) 3))
(define (nats) 
  (letrec ([f (lambda (n)
                (cons n (lambda () (f (+ 1 n)))))])
    (f 1)))
...
(define tests
  (test-suite
   ...
   (check-equal? (sequence -1 -1 0) null "Sequence test:Invalid stride")
   (check-equal? (sequence 3 1 1) null "Sequence test: Invalid start and end")
   ...
   (check-equal? (string-append-map
                   null ".jpg") null "string-append-map: empty list")
   (check-equal? (string-append-map
                   (list "s") "") '("s") "string-append-map: empty suffix") 
   ; list-nth-mod test
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 2) 2 "list-nth-mod test")
   (check-equal? (list-nth-mod (list 0 21 2 3 4) 20) 0 "list-nth-mod test")
   (check-equal? (list-nth-mod (list 0 ) 3) 0 "list-nth-mod test")
   ...
   ; cached-assoc tests
   (check-equal? (cache 3) (cons 3 4) "cached-assoc test")
   (check-equal? (cache 3) (cons 3 4) "cached-assoc test")
   (check-equal? (cache 1) (cons 1 2) "cached-assoc test")
   (check-equal? (cache 1) (cons 1 2) "cached-assoc test")
   (check-equal? (cache 3) (cons 3 4) "cached-assoc test")
   ; while-less test
