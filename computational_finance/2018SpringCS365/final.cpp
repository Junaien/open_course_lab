...
int impliedV(double B_target, double t0,
                double & v, int & num_iter, BinomialTree & binom,
                int n, const Derivative * p_derivative, const Database * p_db,
                double S){
  ...
  double v_high = 2.0;
  double B_v_low = 0;
  binom.FairValue(n, p_derivative, p_db, S, v_low,t0 ,B_v_low);
  ...
  double B_v_high = 0;
  binom.FairValue(n, p_derivative, p_db, S, v_high,t0 ,B_v_high);
  ...
  for(num_iter = 1; num_iter < max_iter; ++num_iter){
    v = (v_low + v_high)/2.0;
    double B = 0;
    binom.FairValue(n, p_derivative, p_db, S, v,t0 ,B);
